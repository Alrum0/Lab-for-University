...
class Program
{
    ...
    static void DisplayProductsTable(IContainer<IName> products)
    {
        if (products.Count == 0)
        {
    ...
    static void ShowHelpMenu(IContainer<IName> products)
{
    while (true)
    {
        Console.Clear();
...
static void ShowEditMenu(IContainer<IName> products, int additionOrder)
{
    ...
    while (true)
    {
        ...
        DisplayProductsTable(tempLinkedListContainer);
    }
}

// Перевантажений варіант для пошуку за ім'ям
static void ShowEditMenu(ArrayContainer<IName> products, string name)
{
    int order = products.GetAdditionOrder(products[name]);
    ShowEditMenu(products, order);
}

static void GeneratorOperationsMenu(IContainer<IName> container)
{
    while (true)
    {
        Console.Clear();
        ...
        switch (choice)
        {
            case "1": 
                ...
                DisplayProductsTableForGenerate(container.GetReverseOrder(), true);
                break;
            ...
            case "2": 
                ...
                if (!string.IsNullOrWhiteSpace(substring))
                {
                    ...
                    DisplayProductsTableForGenerate(container.GetItemsWithSubstring(substring));
                }
                else
                {
            ...
            case "3": 
                ...
                DisplayProductsTableForGenerate(container.GetOrderedByName());
                break;
                
            case "4": 
                Console.WriteLine($"\n{BRIGHT_YELLOW}Товари відсортовані за ціною:{RESET}");
                DisplayProductsTableForGenerate(container.GetOrderedByPrice());
                break;
...
static void DisplayProductsTableWithForeach(IContainer<IName> products)
{
...
// static void DisplayProductsTableForGenerate(IEnumerable<IName> products)
// {
...
//     {
//         string num = counter.ToString();
//         string name = TruncateString(product.Name, col2Width);
//         string price = product.Price.ToString("N2");
//         string type = "-";
//         string mechanism = "-";
//         string brand = "-";
//         string material = "-";
...
//         }
//         
//         mechanism = TruncateString(mechanism, col5Width);
//         brand = TruncateString(brand, col6Width);
//         material = TruncateString(material, col7Width);
//         features = TruncateString(features, col9Width);
//         
//         Console.WriteLine(
//             $"| {num.PadRight(col1Width)} " +
//             $"| {name.PadRight(col2Width)} " +
...
static void DisplayProductsTableForGenerate(IEnumerable<IName> products, bool reverseOrder = false)
{
    var currentProducts = products.ToList();
    ...
    int counter = reverseOrder ? currentProducts.Count : 1;
...
static string TruncateString(string input, int maxLength)
{
    ...
    return input.Length <= maxLength ? input.PadRight(maxLength) : input.Substring(0, maxLength - 3) + "...";
}
    ...
    static void Main()
    {
        ...
        while (!exit)
        {
            ...
            Console.WriteLine($"{RED}9. Вихід{RESET}");
            ...
            switch (choice)
            {
                ...
                case "7":
                    ...
                    DisplayProductsTableWithForeach(products);
                    break;
                case "8":
                    GeneratorOperationsMenu(products);
                    break;
                case "9":
                    exit = true;
